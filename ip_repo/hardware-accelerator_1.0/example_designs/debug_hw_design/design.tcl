...
proc create_ipi_design { offsetfile design_name } {
	...
	# Create instance: hardware-accelerator_0, and set properties
	set hardware-accelerator_0 [ create_bd_cell -type ip -vlnv user.org:user:hardware-accelerator:1.0 hardware-accelerator_0 ]
	...
	# Connect all clock & reset of hardware-accelerator_0 slave interfaces..
	connect_bd_intf_net [get_bd_intf_pins axi_peri_interconnect/M00_AXI] [get_bd_intf_pins hardware-accelerator_0/registers]
	connect_bd_net [get_bd_pins hardware-accelerator_0/registers_aclk] [get_bd_pins sys_clk_0/clk_out1]
	connect_bd_net [get_bd_pins hardware-accelerator_0/registers_aresetn] [get_bd_pins sys_reset_0/peripheral_aresetn]

	# Connect all clock, reset & status pins of hardware-accelerator_0 master interfaces..
	connect_bd_intf_net [get_bd_intf_pins axi_mem_interconnect/S00_AXI] [get_bd_intf_pins hardware-accelerator_0/data]
	connect_bd_net [get_bd_pins hardware-accelerator_0/data_aclk] [get_bd_pins sys_clk_0/clk_out1]
	connect_bd_net [get_bd_pins hardware-accelerator_0/data_aresetn] [get_bd_pins sys_reset_0/peripheral_aresetn]
	connect_bd_net [get_bd_pins hardware-accelerator_0/data_txn_done] [get_bd_pins xlconcat_0/In0]
	connect_bd_net [get_bd_pins hardware-accelerator_0/data_error] [get_bd_pins xlconcat_0/In1]
	connect_bd_net [get_bd_pins hardware-accelerator_0/data_init_axi_txn] [ get_bd_pins axi_gpio_out/gpio_io_o ]


	# Auto assign address
	assign_bd_address

	# Configure address param & range of hardware-accelerator_0 master interfaces..
	set_property range 16K [get_bd_addr_segs {jtag_axi_0/Data/SEG_axi_bram_ctrl_0_Mem0}]
	set_property range 16K [get_bd_addr_segs {hardware-accelerator_0/data/SEG_axi_bram_ctrl_0_Mem0}]
	set_property -dict [list  CONFIG.C_data_TARGET_SLAVE_BASE_ADDR {0xC0000000} ] [get_bd_cells hardware-accelerator_0]

	# Copy all address to hardware-accelerator_v1_0_include.tcl file
	set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
	upvar 1 $offsetfile offset_file
	set offset_file "${bd_path}/hardware-accelerator_v1_0_include.tcl"
	...
	set offset [get_property OFFSET [get_bd_addr_segs /jtag_axi_0/Data/SEG_hardware-accelerator_0_registers_* ]]
	puts $fp "set registers_addr ${offset}"

	close $fp
}

# Set IP Repository and Update IP Catalogue 
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores user.org:user:hardware-accelerator:1.0]]]]
set hw_test_file ${ip_path}/example_designs/debug_hw_design/hardware-accelerator_v1_0_hw_test.tcl
...
for { set i 1 } { 1 } { incr i } {
	set design_name "hardware-accelerator_v1_0_hw_${i}"
