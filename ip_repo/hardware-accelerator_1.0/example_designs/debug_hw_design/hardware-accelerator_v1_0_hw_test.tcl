# Runtime Tcl commands to interact with - hardware-accelerator_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/hardware-accelerator_v1_0_include.tcl
...
# Test: registers
# Create a write transaction at registers_addr address
create_hw_axi_txn w_registers_addr [get_hw_axis $jtag_axi_master] -type write -address $registers_addr -data $wdata_1
# Create a read transaction at registers_addr address
create_hw_axi_txn r_registers_addr [get_hw_axis $jtag_axi_master] -type read -address $registers_addr
# Initiate transactions
run_hw_axi r_registers_addr
run_hw_axi w_registers_addr
run_hw_axi r_registers_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_registers_addr]]
# Compare read data
if { $rdata_tmp == $wdata_1 } {
	puts "Data comparison test pass for - registers"
} else {
	puts "Data comparison test fail for - registers, expected-$wdata_1 actual-$rdata_tmp"
	inc ec
}
...
# Test: data
set wdata_data [get_init_data 0]
create_hw_axi_txn w_data_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_data}
create_hw_axi_txn r_data_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_data_addr
run_hw_axi w_data_addr
run_hw_axi r_data_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_data_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 1 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - data"
} else {
	puts "Data comparison test fail for - data, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
